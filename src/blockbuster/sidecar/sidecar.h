...
class BlockbusterLaunchDialog: public QDialog, 
  ...
  BlockbusterLaunchDialog(SideCar *sidecar, QString host, QString port, QString file, connectionState state, QString rshCmd, QProcess *process, long bbVerbose): 
    mSidecar(sidecar), mState(state), mProcess(process), mBlockbusterPort(port.toInt()) {
    setupUi(this); 
    ...
    initComboBox(fileNameComboBox, "fileNameComboBox.history", file); 
 ...
 signals:   
  ...
  void initComboBox(QComboBox *box, QString filename, QString initialItem);   
