...
int main (int argc, char *argv[]){
  string moviename = "/Users/cook47/dataAndImages/langerGZ.sm";
  
  smSetVerbose(4); 
  BufferedStreamingMovie sm(moviename); 
  if (!sm.ReadHeader()) {
    cerr << "Error: could not read header from movie " <<  moviename << endl;
    exit(1); 
  }
