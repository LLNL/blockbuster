...
int main(int argc, char *argv[])
{
  ...
  for (arg=1; arg<argc && argv[arg][0]=='-'; arg++) {
    if (strncmp(argv[arg], "-h", 2) == 0) {
      ...
      if (arg+1 >= argc) usage(argv[0]);
      nloops=atoi(argv[arg+1]);
      arg++;
    } else if (strcmp(argv[arg], "-pan") == 0) {
      smdbprintf(0,"Panning selected\n");
      if (arg+6 >= argc) usage(argv[0]);
      dpos[0] = atof(argv[++arg]);
      dpos[1] = atof(argv[++arg]);
      ddim[0] = atof(argv[++arg]);
      ddim[1] = atof(argv[++arg]);
      dstep[0] = atof(argv[++arg]);
      dstep[1] = atof(argv[++arg]);
    } else if (strcmp(argv[arg],"-rect") == 0) {
      if (arg+6 >= argc) usage(argv[0]);
      fltpos[0] = atof(argv[++arg]);
      fltpos[1] = atof(argv[++arg]);
      fltdim[0] = atof(argv[++arg]);
      fltdim[1] = atof(argv[++arg]);
      fltstep[0] = atof(argv[++arg]);
      fltstep[1] = atof(argv[++arg]);
      bHaveRect = 1;
    } else if (strcmp(argv[arg], "-range") == 0) {
      if (arg+2 >= argc) usage(argv[0]);
      range[0] = atoi(argv[++arg]);
      range[1] = atoi(argv[++arg]);    
    } else if (strcmp(argv[arg],"-v") == 0) {
      gVerbose = atoi(argv[++arg]); 
  ...
  for (arg; arg < argc; arg++) {
    smdbprintf(0, "\n\nTesting movie: %s\n",argv[arg]);
    sm = smBase::openFile(argv[arg], O_RDONLY, nthreads);
    ...
    // Double check
    int errnum = 0;
    if ((pos[0]<0) || (pos[0]>=sm->getWidth())) errnum = 1;
    if ((pos[1]<0) || (pos[1]>=sm->getWidth())) errnum = 2;
    if ((step[0]<0) || (step[1]<0)) errnum = 3;
    if ((dim[0]<1) || (dim[0]+pos[0]*step[0]>sm->getWidth())) {
      smdbprintf(0, "LH = %d, RH = %d\n", dim[0]+pos[0]*step[0], sm->getWidth()); 
      errnum = 4;
    }
    if ((dim[1]<1) || (dim[1]+pos[1]*step[1]>sm->getHeight())) errnum = 5;
