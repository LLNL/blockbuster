		...
		int max_queue_size, int do_not_block_when_full)
{
  ...
  for (i = 0; i != num_worker_threads; i++) {
    if ((rtn = pthread_create( &(tpool->threads[i]),
		      NULL, pt_pool_thread, (void *)tpool)) != 0)
      fprintf(stderr,"pthread_create %d",rtn), exit(1);
  }

  /* create the work "bucket" list */
  for (i=0; i < max_queue_size; i++) {
    if ((workp = (pt_pool_work_t *)malloc(sizeof(pt_pool_work_t))) == NULL)
      perror("malloc"), exit(1);
    workp->next = tpool->free_queue;
    tpool->free_queue = workp;
  }
}
...
int pt_pool_add_work( pt_pool_t tpool, void (*routine)(void *), void *arg)
{
  ...
  /* wait until it can be added to the queue */
  ...
  /* get a work bucket from the free queue */
  workp = tpool->free_queue;
  tpool->free_queue = tpool->free_queue->next;
...
void pt_pool_destroy(pt_pool_t tpool, int finish)
{
  ...
  while(tpool->free_queue != NULL) {
    cur_nodep = tpool->free_queue;
    tpool->free_queue = tpool->free_queue->next;
    free(cur_nodep);
  }
...
void *pt_pool_thread(void *arg)
{
  pt_pool_t tpool = (pt_pool_t)arg; 
  int            rtn;
  pt_pool_work_t *my_workp;
  void           *lcl_arg;
  void           (*lcl_rout)(void *);
  ...
  for(;;) {
    ...
    /* grab the routine and arg, return the bucket to the free list */
    lcl_rout = my_workp->routine;
    lcl_arg = my_workp->arg;
    my_workp->next = tpool->free_queue;
    tpool->free_queue = my_workp;
